...
function initBabylonScene() {
    ...
    var canvas = document.getElementById("canvas-02");
    ...
    function blenderCameraAnimation(scene) {
        ...
        new BABYLON.SceneLoader.ImportMeshAsync('','models/','tunnel02.gltf', scene).then(() => {
            //scene.createDefaultCameraOrLight(true, true, true)
            //console.log("scene =" + scene);
    
            let camera_active = scene.getCameraByName("Camera")
            scene.activeCamera = camera_active
    
            let cameraAnimation = scene.getAnimationGroupByName("Action.001");
            //stop animation loop
            cameraAnimation.play(false);
            //animatio play speed
            //cameraAnimation.speedRatio = 2;
    ...
    var videoDistanceFromTop = $('#babylon-container-02').offset().top;
    
    $(window).on('scroll', function () {
        var scrollFromTop = $(window).scrollTop();
        var currentVidDistanceFromTop = ($('#babylon-container-02').offset().top - scrollFromTop);
    ...
    document.getElementById("babylon-content-container-02").style.height = videoDurationInPx + windowHeight;
    ...
    document.getElementById("babylon-spacer0001-02").style.marginTop = overlayText01DisFromTop + "px";
    ...
    let text01Scene = new ScrollMagic.Scene({
        triggerHook: 0.5,
        triggerElement: "#babylon-trigger1-02",
        duration: text01Stay,
    })
        .setPin("#babylon-pin1-02")
        .addTo(controller)
    ...
    let text02Stay = 600;
    document.getElementById("babylon-spacer0102-02").style.marginTop = (text02TargetShowTime - text01TargetShowTime) * yOffsetToVidTimeRate - text01Stay + "px"
    
    let text02Scene = new ScrollMagic.Scene({
        triggerHook: 0.5,
        triggerElement: "#babylon-trigger2-02",
        duration: text02Stay,
    })
        .setPin("#babylon-pin2-02")
        .addTo(controller)
    ...
    let text03Stay = 1000;
    document.getElementById("babylon-spacer0203-02").style.marginTop = (text03TargetShowTime - text02TargetShowTime) * yOffsetToVidTimeRate - text02Stay + "px";
    
    let text03Scene = new ScrollMagic.Scene({
        triggerHook: 0.5,
        triggerElement: "#babylon-trigger3-02",
        duration: text03Stay,
    })
        .setPin("#babylon-pin3-02")
        .addTo(controller)
    ...
    let text04Stay = 1000;
    document.getElementById("babylon-spacer0304-02").style.marginTop = (text04TargetShowTime - text03TargetShowTime) * yOffsetToVidTimeRate - text03Stay + "px";
    
    let text04Scene = new ScrollMagic.Scene({
        triggerHook: 0.5,
        triggerElement: "#babylon-trigger4-02",
        duration: text04Stay,
    })
        .setPin("#babylon-pin4-02")
        .addTo(controller)
